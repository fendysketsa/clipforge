...
- Clip Pendek otomatis memilih backsong instrumental dari katalog lokal hasil unduhan resmi YouTube Studio Audio Library berdasarkan tema/mood. Hanya track `Attribution not required` dengan manifest valid yang diterima; dialog memakai gain 80%, musik maksimal 20%, lalu sidechain ducking menurunkannya lagi ketika ada ucapan. Jika katalog belum berisi track cocok, klip Islami tetap dapat memakai fallback orisinal "Cahaya Hikmah" yang disintesis lokal dan berlisensi CC0. Impor track memakai `python scripts/import-youtube-audio-track.py ...`; renderer tidak melakukan scraping Studio.
