...
- Mode `original_rebuild` memakai sumber berizin hanya sebagai bahan riset transkrip. AI wajib aktif; tidak ada fallback ke transkrip mentah. Naskah yang melewati `ORIGINAL_REBUILD_MAX_VERBATIM_WORDS` kata sama berurutan ditulis ulang maksimal sekali lalu ditolak. Media kerja hasil download, audio ekstraksi, dan transkrip penuh dibersihkan sebelum `long_animate` membuat media baru; file lokal milik pemanggil tidak pernah diteruskan ke renderer. Sidecar tetap mewajibkan review fakta, hak provider, disclosure AI, upload Private, dan YouTube Checks.
- Original Rebuild wajib menerima minimal delapan kata perspektif manusia serta referensi bukti hak sumber dan provider. Renderer menyimpan `asset_license_ledger` per kelas aset dan `channel_authenticity_contract`; preflight membandingkan shingle narasi dengan maksimal 20 upload terakhir dan menolak kemiripan di atas `YOUTUBE_GENERATED_CONTENT_MAX_SIMILARITY` (default 0,58). Musik saat ini dibuat prosedural tanpa rekaman pihak ketiga—bukan diklaim sebagai Audio Library—sehingga Checks tetap wajib. Semua upload otomatis dipaksa Private-first meskipun request meminta public.
