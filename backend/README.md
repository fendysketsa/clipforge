...
- setiap Short dan long-form hanya menampilkan watermark `@ryuundy` sebagai signature seal transparan hijau-emas yang ringkas di tengah; ID audit deterministik, metadata provenance Fendy Clipper di MP4, dan hash SHA-256 tetap disimpan tanpa memenuhi layar video; watermark tidak dijadikan bukti kepemilikan sumber dan klaim tetap dibatasi pada kontribusi editorial
...
- skor FYP 85 dipakai untuk ranking dan gate auto-upload batch YouTube, bukan sebagai klaim algoritma YouTube atau safety rule; shortlist diperbaiki otomatis, kandidat yang mencapai target diprioritaskan, dan bila semuanya di bawah target maksimal tiga kandidat yang tetap lolos gate struktur/retensi/konteks dirender untuk review manual
...
- setiap Short mendapat kartu ayat Al-Qur'an 2,8 detik setelah ucapan selesai; kompilasi long-form hanya sekali pada akhir bab terakhir. Wording berasal dari katalog terkunci Terjemahan Kemenag RI 2019, menyimpan URL ayat resmi, dipilih acak-stabil berdasarkan topik, dan tidak dibuat bebas oleh AI. Nonaktifkan dengan `QURAN_OUTRO_ENABLED=false`.
