...
export function ControlPanel({
  ...
  return (
    <details className="panel controlPanel productionSettings optionalPanel" id="production-settings">
      <summary className="optionalPanelSummary">
        ...
        <span className="optionalPanelSnapshot" aria-label="Ringkasan pengaturan aktif">
          <span><Gauge size={13} /> {selectedQuality?.label ?? "Standar"}</span>
          <span>{isLongStory ? `${targetMinutes} menit` : `${minDuration}–${maxDuration} dtk`}</span>
      ...
      <div className="optionalPanelBody productionPanelBody">
        <div className="productionContextBar">
          <span className="activeFormatBadge">
            {isLongStory ? <Film size={15} /> : <Scissors size={15} />}
            {isLongStory ? "Long Story · 16:9" : "Clip Pendek · 9:16"}
          </span>
          <p>Semua perubahan di bawah bersifat opsional dan langsung diterapkan ke proses berikutnya.</p>
        </div>

        <div className="settingsAccordion">
          <details className="settingDisclosure">
            <summary>
              <span className="settingDisclosureIndex">01</span>
              <span className="settingDisclosureCopy">
                <strong>Kualitas & durasi</strong>
                <small>{selectedQuality?.help}</small>
              </span>
              <span className="settingDisclosureValue">
                {selectedQuality?.label} · {isLongStory ? `${targetMinutes} menit` : `${minDuration}–${maxDuration} detik`}
              </span>
              <ChevronDown size={16} />
            </summary>
            <div className="settingDisclosureBody">
          ...
          <details className="settingDisclosure">
            <summary>
              <span className="settingDisclosureIndex">02</span>
              <span className="settingDisclosureCopy">
                <strong>{isLongStory ? "Komposisi video" : "Framing vertikal"}</strong>
                <small>{isLongStory ? "Rasio landscape dijaga otomatis." : "AI menjaga subjek di area aman 9:16."}</small>
              </span>
              <span className="settingDisclosureValue">
                {isLongStory ? "Sinematik otomatis" : CROP_OPTIONS.find((option) => option.value === cropMode)?.label}
              </span>
              <ChevronDown size={16} />
            </summary>
            <div className="settingDisclosureBody">
          ...
          <details className="settingDisclosure">
            <summary>
              <span className="settingDisclosureIndex">03</span>
              <span className="settingDisclosureCopy">
                <strong>Subtitle & otomatisasi</strong>
                <small>Aktifkan hanya fitur tambahan yang dibutuhkan.</small>
              </span>
              <span className="settingDisclosureValue">
                {[burnSubtitles && "Subtitle", aiEnabled && "AI", autoUploadYoutube && "YouTube"].filter(Boolean).join(" · ") || "Semua nonaktif"}
              </span>
              <ChevronDown size={16} />
            </summary>
            <div className="settingDisclosureBody">
              <div className="productionToggleGrid">
                ...
                <ToggleCard
                  ...
                  description="Masuk antrean setelah review"
              ...
              {burnSubtitles ? (
                <details className="captionDisclosure">
                  <summary><span><Captions size={16} /> Gaya subtitle lanjutan</span><ChevronDown size={16} /></summary>
