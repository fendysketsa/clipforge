import { CheckCircle2, ChevronDown, Clock3, ExternalLink, Loader2, Radar, Search, ShieldCheck } from "lucide-react";
import type {
  AutoViralRun,
...
export function AutoViralPanel({
  ...
  const activityLabel = isSearching
    ? "Mencari kandidat…"
    : isRunning
      ? `Berjalan ${progress}%`
      : sources.length
        ? `${sources.length} kandidat`
        : "Siap digunakan";

  return (
    <details className="panel autoViralPanel optionalPanel optionalPanel--violet">
      ...
      <div className="optionalPanelBody autoViralBody">
