...
log "Membangun ulang backend dengan akses RTX/NVIDIA"
cd -- "${PROJECT_DIR}"
docker compose \
...
log "Memverifikasi nvidia-smi dari dalam backend"
docker compose \
  -f docker-compose.yml \
  -f docker-compose.gpu.yml \
  exec -T backend nvidia-smi

printf '\n\033[1;32m[clipforge:nvidia:ready]\033[0m GPU NVIDIA sudah terlihat dari backend. Refresh UI setelah job aktif.\n'
